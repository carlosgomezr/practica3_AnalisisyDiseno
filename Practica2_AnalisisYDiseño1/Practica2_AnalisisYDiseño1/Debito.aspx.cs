    ...
    public partial class Debito : System.Web.UI.Page
    {
        ...
        protected void b_agua_Click(object sender, EventArgs e)
        {
            ...
            if(BaseDatos.ValidarDatosPagos(cuenta_ser.Text,monto.Text))
            {
                ...
                //Se hace el debito
                int estado;
                estado = BaseDatos.Debitar(Int32.Parse(Session["cuenta_"].ToString()), Int32.Parse(cuenta_ser.Text), float.Parse(monto.Text), descripcion.Text);
                if (estado == 1)
                {
                    Ms_error1.Text = "Debito Realizado Correctamente";
                    Ms_error1.Visible = true;

                }
                else if (estado == -1)
                {
                    Ms_error1.Text = "La cuenta a debitar no tiene fondos suficientes";
                    Ms_error1.Visible = true;
                }
                else if (estado == 2)
                {
